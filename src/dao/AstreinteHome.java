...
public class AstreinteHome {
	...
	public Astreinte merge(Astreinte detachedInstance) {
		...
		try {
			Astreinte result = (Astreinte) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Astreinte findById(java.math.BigDecimal id) {
		log.debug("getting Astreinte instance with id: " + id);
		try {
			Astreinte instance = (Astreinte) sessionFactory.getCurrentSession().get("dao.Astreinte", id);
			if (instance == null) {
	...
	public List findByExample(Astreinte instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("dao.Astreinte")
